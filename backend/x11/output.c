...
void handle_present_event(struct wlr_x11_backend *x11, xcb_present_generic_event_t *e) {
	...
	case XCB_PRESENT_CONFIGURE_NOTIFY:
		...
		output = output_from_window(x11, configure->window);
	...
	case XCB_PRESENT_COMPLETE_NOTIFY:
		...
		output = output_from_window(x11, complete->window);

		// This can happen after window is destroyed
		if (!output) {
			return;
		}
	...
	case XCB_PRESENT_IDLE_NOTIFY:
		...
		output = output_from_window(x11, idle->window);
...
static bool output_present(struct wlr_output *wlr_output, struct wlr_image *img,
	...
	struct wlr_x11_output *output = get_x11_output_from_output(wlr_output);
	struct wlr_x11_backend *x11 = output->x11;
	xcb_pixmap_t pixmap = (xcb_pixmap_t)(uintptr_t)img->backend_priv;
...
struct wlr_output *wlr_x11_output_create(struct wlr_backend *backend) {
	...
	output_set_refresh(wlr_output, 0);
	...
	xcb_create_window(x11->xcb, XCB_COPY_FROM_PARENT, output->win,
		x11->root, 0, 0, wlr_output->width, wlr_output->height, 1,
		XCB_WINDOW_CLASS_INPUT_OUTPUT, x11->visual, mask, values);
