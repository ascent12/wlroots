...
static size_t parse_outputs_env(const char *name) {
	...
	char *end;
	unsigned long outputs = strtoul(outputs_str, &end, 10);
	if (errno || end == outputs_str || *end) {
	...
	if (strcmp(name, "wayland") == 0) {
		...
		if (!*session) {
			*session = wlr_session_create(display);
			if (!*session) {
				wlr_log(WLR_ERROR, "Failed to start session");
				return NULL;
			}
		}
...
struct wlr_backend *wlr_backend_autocreate(struct wl_display *display) {
	...
	if (!backend) {
		wlr_log(WLR_ERROR, "Could not create multi backend");
		return NULL;
	}
